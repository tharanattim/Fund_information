...
function loadTable (){
	table = $('#dtb_queue').DataTable( {
		...
		"ajax": {
			url: "/rmuti-electronicqueue/Queue/getAllQueueByDeptAndDate/03/" + moment().format("YYYY-MM-DD"),
			dataSrc: ""
		},
		"columns": [
			{ "data": "queue_no" },
...
function renderStatus() {
	$.ajax({
		url : "/rmuti-electronicqueue/Department/getStaustByid/03",
		type : 'GET',
		async : true
	}).done(function(data) {
...
function updateSystemStatus(status) {
	...
	$.ajax({
		url : "/rmuti-electronicqueue/Department/updateDepartment/03",
		type : 'PUT',
...
function renderQueueLength(){
	$.ajax({
		url : "/rmuti-electronicqueue/Queue/getCountQueueByDeptAndDate/03/" + moment().format("YYYY-MM-DD"),
		type : 'GET',
		async : true
	}).done(function(data) {
		var result = JSON.parse(data);
...
function getNextQueue(){
	$.ajax({
		url : "/rmuti-electronicqueue/Queue/getNextQueueByByDeptAndDate/03/" + moment().format("YYYY-MM-DD"),
		type : 'GET',
		async : false
	}).done(function(data) {
		var result = JSON.parse(data);
	...
	Array.prototype.filter.call(forms, function(form) {
		$('#btn_pause_queue').click(function() {
			if (form.checkValidity() === false) {
				...
				$.ajax({
					url : "/rmuti-electronicqueue/Queue/updatePauseTime/03/",
					type : 'PUT',
					data : JSON.stringify(data),
					contentType: "application/json; charset=utf-8",
					async : false
...
function pauseQueue(){
	if ($('#btn_pause').text() == 'พักคิว'){
		...
		updateSystemStatus('01');
		$.ajax({
			url : "/rmuti-electronicqueue/Queue/getPauseTime/03/",
			type : 'GET',
			async : false
		}).done(function(data) {
			var result = JSON.parse(data);
